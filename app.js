...
(function(){
  ...
  // ---- Config / Constants ----
  // PRICE_DT is now a FALLBACK only — each product can define its own `price` in products.json
  const PRICE_DT = 25;
  const SIZE = '50ML';
  const CART_KEY = 'dadios_cart_v1';
  // Helper: each product has its own price now. If missing, use the global fallback.
  function priceOf(p){ return (p && typeof p.price === 'number') ? p.price : PRICE_DT; }
  ...
  // ---- Data ----
  // Products & best-sellers are now loaded from products.json at startup.
  // Edit products.json (or use the admin page) to add/edit perfumes — no need to touch this file anymore.
  let PRODUCTS = [];
  let BEST_SELLERS_IDS = [];

  // Loads products.json. Returns a promise so init can await it.
  function loadProducts() {
    return fetch('products.json', { cache: 'no-cache' })
      .then(function(r){
        if(!r.ok) throw new Error('HTTP '+r.status);
        return r.json();
      })
      .then(function(data){
        if(!data || !Array.isArray(data.products)) throw new Error('Invalid products.json shape');
        PRODUCTS = data.products;
        BEST_SELLERS_IDS = Array.isArray(data.bestSellers) ? data.bestSellers : [];
      })
      .catch(function(err){
        console.error('[DADIOS] Failed to load products.json:', err);
        // Defensive: keep arrays empty so render() shows the empty-state message rather than crashing
        PRODUCTS = [];
        BEST_SELLERS_IDS = [];
      });
  }
  ...
  function render(list){
    ...
    var html = list.map(function(p){
      return '<div class="card"><img src="'+p.thumbImg+'" alt="'+p.name+'" data-id="'+p.id+'" class="thumb"><div class="img-overlay">Cliquez pour voir les détails</div><span class="name">'+p.name+'</span><span class="price">'+priceOf(p)+' DT</span><div class="card-actions"><button class="addBtn" type="button" data-add="'+p.id+'">Ajouter</button></div></div>';
    }).join('');
  ...
  function openPerfumeModalById(id){
    ...
    var pmOrder = $('#pmOrder'); if(pmOrder) pmOrder.href = waLink('Je veux commander '+p.name+' ('+SIZE+' - '+priceOf(p)+' DT)');
  ...
  function addToCart(p){ var cart = loadCart(); var found = cart.find(function(x){return x.id===p.id;}); if(found) found.qty += 1; else cart.push({ id:p.id, name:p.name, img:p.thumbImg, qty:1, price:priceOf(p), size:SIZE }); saveCart(cart); refreshCartUI(); }
  ...
  // ---- Init ----
  // Load products.json FIRST, then populate filters and render. Cart UI is unrelated
  // to products data (it uses localStorage prices baked in at add-to-cart time),
  // so it can render immediately.
  refreshCartUI();
  loadProducts().then(function(){
    populateFilters();
    applyFilters();
    // Re-expose updated PRODUCTS reference (the let was reassigned, so window.DADIOS gets the new array)
    window.DADIOS.PRODUCTS = PRODUCTS;
    window.DADIOS.BEST_SELLERS_IDS = BEST_SELLERS_IDS;
  });

  window.DADIOS = { PRODUCTS: PRODUCTS, BEST_SELLERS_IDS: BEST_SELLERS_IDS, applyFilters: applyFilters, populateFilters: populateFilters, refreshCartUI: refreshCartUI };
